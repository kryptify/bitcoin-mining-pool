    ...
    async.parallel({
        pool: function(callback){
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                if (error){
                    console.log('error', logSystem, 'Error getting redis data %j', [error]);
                    callback(true);
                    return;
                }
        ...
        network: function(callback){
            apiInterfaces.rpcDaemon('getlastblockheader', {}, function(error, reply){
                ...
                if (error){
                    console.log('error', logSystem, 'Error getting daemon data %j', [error]);
                    callback(true);
                    return;
                }
        ...
        console.log('info', logSystem, 'Stat collection finished: %d ms redis, %d ms daemon', [redisFinished - startTime, daemonFinished - startTime]);

        if (error){
            console.log('error', logSystem, 'Error collecting all stats');
        }
        else{
...
function broadcastLiveStats(){
    ...
    console.log('info', logSystem, 'Broadcasting to %d visitors and %d address lookups', [Object.keys(liveConnections).length, Object.keys(addressConnections).length]);
...
function handleStartMining(urlParts, response){
    console.log("dddd");
    StartMining = true;
}
...
function authorize(request, response){
    ...
    console.log('warn', logSystem, 'Admin authorized');
    ...
    async.waterfall([
        ...
        function(callback){
            redisClient.multi([
                ...
                if (error) {
                    console.log('error', logSystem, 'Error trying to get admin data from redis %j', [error]);
                    callback(true);
                    return;
                }
...
var server = http.createServer(function(request, response){
    ...
    switch(urlParts.pathname){
        ...
        case '/get_blocks':
            ...
            break;
        case '/start_mining':
            handleStartMining(urlParts, response);
            break;
        case '/admin_stats':
...
server.listen(config.api.port, function(){
    console.log('info', logSystem, 'API started & listening on port %d', [config.api.port]);
});
