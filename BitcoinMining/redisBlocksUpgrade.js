...
async.series([
    function(callback){
        redisClient.smembers(config.coin + ':blocksUnlocked', function(error, result){
            if (error){
                console.log('error', logSystem, 'Error trying to get unlocke blocks from redis %j', [error]);
                callback();
                return;
            }
            if (result.length === 0){
                console.log('info', logSystem, 'No unlocked blocks in redis');
                callback();
                return;
            }
            ...
            async.map(blocks, function(block, mapCback){
                ...
                if (blocks.length === 0){
                    console.log('info', logSystem, 'No unlocked blocks');
                    callback();
                    return;
                }
                ...
                redisClient.zadd(zaddCommands, function(err, result){
                    if (err){
                        console.console.log('failed zadd ' + JSON.stringify(err));
                        callback();
                        return;
                    }
                    console.console.log('successfully converted unlocked blocks to matured blocks');
                    callback();
                });
    ...
    function(callback){
        redisClient.smembers(config.coin + ':blocksPending', function(error, result) {
            ...
            if (result.length === 0) {
                console.log('info', logSystem, 'No pending blocks in redis');
                callback();
                return;
            }
            ...
            async.map(result, function(item, mapCback){
                ...
                redisClient.zadd(zaddCommands, function(err, result){
                    ...
                    console.console.log('successfully converted pending blocks to block candidates');
                });
